...
try {
    ...
    if ($userId) {
        $q = "SELECT id, uuid, nombre, apellido, email, telefono, creado_en FROM usuarios WHERE id = ? LIMIT 1";
        $stmt = $db->prepare($q);
        $stmt->execute([$userId]);
    } else {
        $q = "SELECT id, uuid, nombre, apellido, email, telefono, creado_en FROM usuarios WHERE email = ? LIMIT 1";
        $stmt = $db->prepare($q);
        $stmt->execute([$email]);
    }
